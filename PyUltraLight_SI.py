...
# Read the initial information from soliton_solution
current_beta = []
current_Lambda_hat = []
with open('./Soliton Profile Files/Info_soliton_solution.txt', 'r') as initial_info:
    for i, line in enumerate(initial_info):
        if i == 0:
            for word in line.split():
                current_Lambda_hat.append(word)
            Lambda_hat = float(current_Lambda_hat[-1])
        elif i == 3:
            for word in line.split():
                current_beta.append(word)
...
def evolve(central_mass, num_threads, length, length_units, resol, duration, duration_units, step_factor, save_number, save_options,
    ...
    for i in range(3, 6):
        if tm[i] in range(0, 10):
            ...
            talt[i - 3] = tm[i]
    timestamp = '{}{}{}{}{}{}{}{}{}{}{}{}{}'.format(tm[0], '.', tm[1], '.', tm[2], '_', talt[0], 'h', talt[1], 'm', talt[2], '_', resol)
    ...
    file.write(('{}{}'.format('Lambda_hat = ', Lambda_hat)))
    file.write('\n')
    ...
    f = np.load('./Soliton Profile Files/initial_fSI.npy')
    ...
    ##########################################################################################
    # COMPUTE INITIAL VALUE OF POTENTIAL
    ...
    for ix in range(actual_num_steps):
        if halfstepornot == 1:
            psi = ne.evaluate("exp(-1j*0.5*h*phisp)*exp(-1j*0.5*h*Lambda_hat*rho)*psi")
            halfstepornot = 0
        else:
            psi = ne.evaluate("exp(-1j*h*phisp)*exp(-1j*h*Lambda_hat*rho)*psi")
        funct = fft_psi(psi)
